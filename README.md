...
### Technical Details:
* **Genomic Context**: Exome sequencing focused exclusively on **Chromosome 20** (GRCh38 reference)[cite: 18].
* **Datasets**: Simulated data derived from real genotypes provided by the 1000 Genomes Project[cite: 5].
* **Workflow**: The pipeline covers the entire process from raw data quality control (FastQC) and alignment (Bowtie2) to variant calling (Freebayes), clinical annotation (VEP), and diagnostic interpretation[cite: 3, 50].
* **Reproducibility**: All intermediate files, logs, and final variants are organized to ensure the analysis is fully reproducible[cite: 68, 69].

## Repository Structure
* **`pipeline_trio_x.sh`**: The original Bash script documenting the complete bioinformatic pipeline.
* **`trio_1/` to `trio_5/`**: Directories containing the filtered VCF files and quality reports for each assigned family case[cite: 22].
